...
const StockTracker = ({ selectedSymbol }) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (typeof window !== 'undefined' && 'Notification' in window) {
          if (Notification.permission === 'granted' && showLoading) {
            new Notification('Stock Data Updated', {
              body: `${selectedSymbol.label}: ₹${response.data.price}`,
  ...
  const calculateCorrection = () => {
    if (!stockData || !selectedSymbol.ath) return null;
    
    const correctionPercent = ((stockData.price - selectedSymbol.ath) / selectedSymbol.ath) * 100;
    const pointsDiff = Math.abs(stockData.price - selectedSymbol.ath);
            ...
            <div className="correction-section">
              <div className={`correction-card ${correction.isBelow ? 'below' : 'above'}`}>
                ...
                <div className="correction-detail">
                  {formatNumber(correction.points)} points {correction.isBelow ? 'below' : 'above'} ₹{formatNumber(selectedSymbol.ath)}
                </div>
              </div>
            </div>
          )}
